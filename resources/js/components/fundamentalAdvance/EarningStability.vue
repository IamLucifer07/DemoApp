...
export default {
    ...
    data() {
        return {
           ...
           tableColumns: [
            ...
            { key: 'mean_Eps', label: 'Mean EPS' },
            { key: 'variance', label: 'Variance' },
             { key: 'standard_deviation', label: 'Standard Deviation' },
            { key: 'earning_stability_ratio', label: 'Earning Stability Ratio' },
            { key: 'stability_category', label: 'Stability Interpretation' }, 
        ],
        };
    },
    async created() {
    ...
    methods: {
        ...
        async fetchTotalAssetsData() {
            ...
            try {
                const response = await axios.get("/api/earning-stability", {
                    params: {
